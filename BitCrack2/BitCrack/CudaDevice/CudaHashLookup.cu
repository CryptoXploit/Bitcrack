...
__constant__ unsigned int _TARGET_XPOINT[MAX_TARGETS_CONSTANT_MEM][8];
__constant__ unsigned int _NUM_TARGET_HASHES_OR_XPOINTS[1];
...
cudaError_t CudaHashLookup::setTargetConstantMemory(const std::vector<struct hash160>& targets)
{
	...
	cudaError_t err = cudaMemcpyToSymbol(_NUM_TARGET_HASHES_OR_XPOINTS, &count, sizeof(unsigned int));
	if (err) {
		return err;
	}
...
/**
Copies the target xpoints to constant memory
*/
cudaError_t CudaHashLookup::setTargetConstantMemory(const std::vector<struct xpoint>& targets)
{
	size_t count = targets.size();

	for (size_t i = 0; i < count; i++) {

		cudaError_t err = cudaMemcpyToSymbol(_TARGET_XPOINT, targets[i].p, sizeof(unsigned int) * 8, i * sizeof(unsigned int) * 8);
...
void CudaHashLookup::initializeBloomFilter(const std::vector<struct xpoint>& targets, unsigned int* filter, unsigned int mask)
{
	// Use the low 16 bits of each word in the xpoint as the index into the bloom filter
	for (unsigned int i = 0; i < targets.size(); i++) {

		for (int j = 0; j < 8; j++) {
			unsigned int idx = targets[i].p[j] & mask;
...
void CudaHashLookup::initializeBloomFilter64(const std::vector<struct xpoint>& targets, unsigned int* filter, unsigned long long mask)
{
	for (unsigned int k = 0; k < targets.size(); k++) {

		//unsigned int hash[5];

		unsigned long long idx[8];

		//undoRMD160FinalRound(targets[k].h, hash);

		idx[0] = ((unsigned long long)targets[k].p[0] << 32 | targets[k].p[1]) & mask;
		idx[1] = ((unsigned long long)targets[k].p[2] << 32 | targets[k].p[3]) & mask;
		idx[2] = ((unsigned long long)(targets[k].p[0] ^ targets[k].p[1]) << 32 | (targets[k].p[1] ^ targets[k].p[2])) & mask;
		idx[3] = ((unsigned long long)(targets[k].p[2] ^ targets[k].p[3]) << 32 | (targets[k].p[3] ^ targets[k].p[4])) & mask;
		idx[4] = ((unsigned long long)(targets[k].p[0] ^ targets[k].p[3]) << 32 | (targets[k].p[1] ^ targets[k].p[3])) & mask;

		idx[5] = ((unsigned long long)(targets[k].p[3] ^ targets[k].p[4]) << 32 | (targets[k].p[4] ^ targets[k].p[5])) & mask;
		idx[6] = ((unsigned long long)(targets[k].p[5] ^ targets[k].p[6]) << 32 | (targets[k].p[6] ^ targets[k].p[7])) & mask;
		idx[7] = ((unsigned long long)(targets[k].p[3] ^ targets[k].p[6]) << 32 | (targets[k].p[4] ^ targets[k].p[6])) & mask;
...
/**
Populates the bloom filter with the target xpoints
*/
cudaError_t CudaHashLookup::setTargetBloomFilter(const std::vector<struct xpoint>& targets)
{
	unsigned int bloomFilterBits = getOptimalBloomFilterBits(1.0e-10, targets.size());
...
cudaError_t CudaHashLookup::setTargets(const std::vector<struct xpoint>& targets)
{
	cleanup();
...
__device__ bool checkBloomFilterHash160(const unsigned int hash[5])
{
...
__device__ bool checkBloomFilterXPoint(const unsigned int point[8])
{
	bool foundMatch = true;
...
__device__ bool checkBloomFilterHash160_64(const unsigned int hash[5])
{
	bool foundMatch = true;
...
__device__ bool checkBloomFilterXPoint_64(const unsigned int point[8])
{
	bool foundMatch = true;
	...
	idx[0] = ((unsigned long long)point[0] << 32 | point[1]) & mask;
	idx[1] = ((unsigned long long)point[2] << 32 | point[3]) & mask;
	idx[2] = ((unsigned long long)(point[0] ^ point[1]) << 32 | (point[1] ^ point[2])) & mask;
	idx[3] = ((unsigned long long)(point[2] ^ point[3]) << 32 | (point[3] ^ point[4])) & mask;
	idx[4] = ((unsigned long long)(point[0] ^ point[3]) << 32 | (point[1] ^ point[3])) & mask;

	idx[5] = ((unsigned long long)(point[3] ^ point[4]) << 32 | (point[4] ^ point[5])) & mask;
	idx[6] = ((unsigned long long)(point[5] ^ point[6]) << 32 | (point[6] ^ point[7])) & mask;
	idx[7] = ((unsigned long long)(point[3] ^ point[6]) << 32 | (point[4] ^ point[6])) & mask;
...
__device__ bool checkHash(const unsigned int hash[5])
{
	...
	if (*_USE_BLOOM_FILTER == 1) {
		return checkBloomFilterHash160(hash);
	}
	else if (*_USE_BLOOM_FILTER == 2) {
		return checkBloomFilterHash160_64(hash);
	}
	else {
		for (int j = 0; j < *_NUM_TARGET_HASHES_OR_XPOINTS; j++) {
			bool equal = true;
			for (int i = 0; i < 5; i++) {
...
__device__ bool checkXPoint(const unsigned int point[8])
{
	bool foundMatch = false;

	if (*_USE_BLOOM_FILTER == 1) {
		return checkBloomFilterXPoint(point);
	}
	else if (*_USE_BLOOM_FILTER == 2) {
		return checkBloomFilterXPoint_64(point);
	}
	else {
		for (int j = 0; j < *_NUM_TARGET_HASHES_OR_XPOINTS; j++) {
			bool equal = true;
			for (int i = 0; i < 8; i++) {
				equal &= (point[i] == _TARGET_XPOINT[j][i]);
			}
